...
def _to_jpeg(im):
    if im.mode != 'RGB':
        im = im.convert('RGB')
    return 'image/jpeg', 'width=%s&height=%s' % (im.size[0], im.size[1]), im.tostring('jpeg', 'RGB')
...
def check_image_size(fcontent, min_width, min_height):
    '''
    check if size less than min_width or min_height.

    >>> pwd = os.path.join(os.path.split(os.path.abspath(__file__))[0], 'tests')
    >>> f1000x600 = _load_from_file(os.path.join(pwd, '1000x600.jpg'))
    >>> check_image_size(f1000x600, 800, 400)
    >>> check_image_size(f1000x600, 800, 700)
    Traceback (most recent call last):
      ...
